...
function render(w){const c=w.querySelector(".content"),t=w.dataset.type,s=w.dataset.text||"";w.querySelector(".title").textContent=w.dataset.title||"";
 ...
 else if(t==="message") c.innerHTML='<div class="messages">'+s.split("\n").filter(Boolean).map(l=>{const a=l.split("|"),me=(a[0]||"").trim()==="나";return `<div class="bubble ${me?"me":""}"><b>${esc((a[0]||"").trim())}</b><br>${esc(a.slice(1).join("|").trim())}</div>`}).join("")+"</div>";
 else if(t==="music"){const a=s.split("|");c.innerHTML=`<div class="musicgrid"><div class="cover">♫</div><div><b>${esc(a[0]||"Song Title")}</b><br><small>${esc(a[1]||"Artist")}</small><div class="progress"><i></i></div></div></div>`}
 ...
 else if(t==="profile"){const a=s.split("|");c.innerHTML=`<div class="profilebox"><div class="avatar"></div><b>${esc(a[0]||"NAME")}</b><p>${esc(a[1]||"status message")}</p></div>`}
...
function buildEditor(el){
 ...
 const t=el.dataset.type;editor.innerHTML=`<h3>${t.toUpperCase()}</h3><label>창 제목<input id="eTitle" value="${esc(el.dataset.title)}"></label><label style="display:block">내용<textarea id="eText" rows="7">${esc(el.dataset.text)}</textarea></label>${t==="photo"?'<label style="display:block">사진<input id="ePhoto" type="file" accept="image/*"></label>':""}<div class="two"><label>너비<input id="eWidth" type="range" min="220" max="620" value="${Math.round(el.getBoundingClientRect().width)}"></label><label>삭제<button id="deleteSelected">창 삭제</button></label></div>${t==="message"?'<p class="sub">메신저: “이름 | 내용”을 한 줄씩 입력. “나 | 내용”은 오른쪽에 표시돼.</p>':""}`;
 ...
 if(t==="photo") $("#ePhoto").onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{el.dataset.img=r.result;render(el)};r.readAsDataURL(f)}
}
...
$("#theme").onchange=e=>desk.className="desktop theme-"+e.target.value;$("#bgColor").oninput=e=>desk.style.backgroundColor=e.target.value;$("#accent").oninput=e=>desk.style.setProperty("--a",e.target.value);
$("#fontFamily").onchange=e=>desk.style.setProperty("--desk-font",e.target.value);
$("#fontSize").oninput=e=>desk.style.setProperty("--desk-font-size",e.target.value+"px");
$("#titleFont").onchange=e=>desk.style.setProperty("--title-font",e.target.value);
$("#bgFit").onchange=e=>desk.style.backgroundSize=e.target.value;$("#dim").oninput=e=>desk.querySelector(".dim").style.opacity=e.target.value/100;$("#opacity").oninput=e=>desk.style.setProperty("--win-alpha",e.target.value/100);$("#radius").oninput=e=>desk.style.setProperty("--radius",e.target.value+"px");$("#clock").oninput=e=>$("#trayClock").textContent=e.target.value;
$("#wallpaper").onchange=e=>{const f=e.target.files[0];if(!f)return;$("#wallpaperName").textContent=f.name;const r=new FileReader();r.onload=()=>desk.style.backgroundImage=`url("${r.result}")`;r.readAsDataURL(f)};
$("#resetBtn").onclick=()=>{if(confirm("현재 꾸민 화면을 초기화할까?"))location.reload()};
...
성격, 직업, 습관, 말투와 현재 관계가 자연스럽게 드러나도록 작성해주세요.]`;try{await navigator.clipboard.writeText(p);toast("OOC가 복사됐어 ♡")}catch{const a=document.createElement("textarea");a.value=p;document.body.appendChild(a);a.select();document.execCommand("copy");a.remove();toast("OOC가 복사됐어 ♡")}};
$("#saveBtn").onclick=async()=>{if(!window.html2canvas){toast("저장 라이브러리를 불러오지 못했어.");return}$$(".selected").forEach(x=>x.classList.remove("selected"));try{await document.fonts.ready;const c=await html2canvas(desk,{scale:3,useCORS:true,backgroundColor:null,logging:false});const a=document.createElement("a");a.download="DEARLY_DESKTOP_MY_PC.png";a.href=c.toDataURL("image/png");a.click();toast("고화질 PNG 저장 완료 ♡")}catch(err){console.error(err);toast("PNG 저장 중 문제가 생겼어.")}};
