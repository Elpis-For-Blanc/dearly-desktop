...
function buildEditor(el){
 ...
 const t=el.dataset.type;editor.innerHTML=`<h3>${t.toUpperCase()}</h3><label>창 제목<input id="eTitle" value="${esc(el.dataset.title)}"></label><label style="display:block">내용<textarea id="eText" rows="7">${esc(el.dataset.text)}</textarea></label>${["photo","music","profile"].includes(t)?`<label style="display:block">${t==="music"?"앨범 사진":t==="profile"?"프로필 사진":"사진"}<input id="ePhoto" type="file" accept="image/*"></label><div id="mediaFileName" class="sub">${el.dataset.fileName?esc(el.dataset.fileName):"선택된 파일이 없습니다."}</div>${el.dataset.img?'<button id="removeMediaPhoto" type="button">사진 제거</button>':""}`:""}<div class="two"><label>너비<input id="eWidth" type="range" min="220" max="620" value="${Math.round(el.getBoundingClientRect().width)}"></label><label>삭제<button id="deleteSelected">창 삭제</button></label></div>${t==="message"?'<p class="sub">메신저: “이름 | 내용”을 한 줄씩 입력. “나 | 내용”은 오른쪽에 표시돼.</p>':""}`;
...
// Messenger direction setting
(()=>{
 const desk=document.querySelector("#desk");
 const sel=document.querySelector("#messageDirection");
 if(!desk||!sel)return;
 const apply=()=>{
   desk.classList.remove("msg-left","msg-right","msg-alternating");
   desk.classList.add("msg-"+sel.value);
 };
