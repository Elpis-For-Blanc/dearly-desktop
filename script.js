...
$("#saveBtn").onclick=async()=>{
  if(!window.html2canvas){toast("저장 기능을 불러오지 못했습니다.");return}
  $$(".selected").forEach(x=>x.classList.remove("selected"));
  try{
    await document.fonts.ready;
    // html2canvas가 일부 최신 CSS 효과를 해석하지 못해 저장이 실패하는 것을 방지합니다.
    desk.classList.add("png-exporting");
    const c=await html2canvas(desk,{
      scale:3,
      useCORS:true,
      allowTaint:false,
    ...
    a.download="DEARLY_DESKTOP_MY_PC.png";
    a.href=url;
    document.body.appendChild(a);
    a.click();
    a.remove();
    setTimeout(()=>URL.revokeObjectURL(url),1500);
    toast("고화질 PNG 저장이 완료되었습니다.");
  }catch(err){
    console.error("PNG export failed:",err);
    toast("PNG 저장 중 문제가 발생했습니다.");
  }finally{
    desk.classList.remove("png-exporting");
  }
};
...
// ===== PNG sticker / floating photo =====
(()=>{
  const stickerBtn=document.querySelector('#addPngSticker');
  const photoBtn=document.querySelector('#addFloatingPhoto');
  const stickerInput=document.querySelector('#pngStickerInput');
  const photoInput=document.querySelector('#floatingPhotoInput');
  if(!stickerBtn||!photoBtn||!stickerInput||!photoInput)return;

  function addImageDeco(src,name,kind){
    const el=document.createElement('div');
    el.className='deco image-deco '+(kind==='sticker'?'png-sticker':'floating-photo');
    el.dataset.kind='image-deco';
    el.dataset.imageKind=kind;
    el.dataset.fileName=name||'';
    el.style.left=(56+(di*4)%25)+'%';
    el.style.top=(12+(di*6)%52)+'%';
    el.style.zIndex=++z; di++;
    const img=document.createElement('img'); img.src=src; img.alt='';
    el.appendChild(img); decos.appendChild(el); makeDraggable(el);
    el.addEventListener('pointerdown',()=>select(el));
    select(el);
  }
  function read(input,kind){
    const f=input.files&&input.files[0]; if(!f)return;
    const r=new FileReader(); r.onload=()=>addImageDeco(r.result,f.name,kind); r.readAsDataURL(f);
    input.value='';
  }
  stickerBtn.onclick=()=>stickerInput.click();
  photoBtn.onclick=()=>photoInput.click();
  stickerInput.onchange=()=>read(stickerInput,'sticker');
  photoInput.onchange=()=>read(photoInput,'photo');

  const oldBuild=buildEditor;
  buildEditor=function(el){
    if(el?.dataset?.kind!=='image-deco') return oldBuild(el);
    const photo=el.dataset.imageKind==='photo';
    const w=Math.round(el.getBoundingClientRect().width);
    const h=Math.round(el.getBoundingClientRect().height);
    editor.innerHTML=`<h3>${photo?'FLOATING PHOTO':'PNG STICKER'}</h3>
      <p class="sub">${esc(el.dataset.fileName||'이미지')}</p>
      <label>너비<input id="imgDecoW" type="range" min="40" max="520" value="${w}"></label>
      <label>높이<input id="imgDecoH" type="range" min="40" max="520" value="${h}"></label>
      ${photo?'<label><input id="imgDecoFrame" type="checkbox" '+(!el.classList.contains('no-frame')?'checked':'')+'> 사진 프레임 표시</label>':''}
      <button id="deleteSelected">삭제</button>`;
    document.querySelector('#imgDecoW').oninput=e=>el.style.width=e.target.value+'px';
    document.querySelector('#imgDecoH').oninput=e=>el.style.height=e.target.value+'px';
    const frame=document.querySelector('#imgDecoFrame');
    if(frame) frame.onchange=e=>el.classList.toggle('no-frame',!e.target.checked);
    document.querySelector('#deleteSelected').onclick=()=>{el.remove();resetEditor()};
  };
})();
