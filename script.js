...
function render(w){const c=w.querySelector(".content"),t=w.dataset.type,s=w.dataset.text||"";w.querySelector(".title").textContent=w.dataset.title||"";
 if(t==="note"||t==="sticky"||t==="terminal") c.textContent=s;
   ...
   const lines=s.split("\n").filter(Boolean);
   let checked=[];
   try{checked=JSON.parse(w.dataset.todoChecked||"[]")}catch(e){checked=[]}
   if(!Array.isArray(checked))checked=[];
   c.innerHTML=lines.map((x,i)=>`<button type="button" class="todo-line ${checked[i]?"done":""}" data-todo-index="${i}"><span class="todo-box" aria-hidden="true">${checked[i]?"✓":""}</span><span class="todo-text">${esc(x)}</span></button>`).join("");
   c.querySelectorAll(".todo-line").forEach(row=>{
     row.addEventListener("click",e=>{
       e.preventDefault();
       e.stopPropagation();
       const i=Number(row.dataset.todoIndex);
       let state=[];
       try{state=JSON.parse(w.dataset.todoChecked||"[]")}catch(err){state=[]}
       if(!Array.isArray(state))state=[];
       state[i]=!state[i];
       w.dataset.todoChecked=JSON.stringify(state);
       row.classList.toggle("done",!!state[i]);
       const box=row.querySelector(".todo-box");
       if(box)box.textContent=state[i]?"✓":"";
     });
   });
 }
